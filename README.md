...
You can pass C++ variables by pointers into cxtpl, that is very useful if you want to use complex data structures as template parameters.

C++ template engine transpiles template into C++ code, so you will gain VERY good performance and the full power of C++.

C++ template engine may be run in two modes:
...
+ cling-mode (C++ JIT executed at runtime): compile cxtpl code into C++ file or std::string, then run generated code in Cling interpreter (no need to recompile app, useful in dev-mode or for php-style apps). See example https://github.com/blockspacer/CXXCTP

## How to use C++ file generated by CXTPL

Again, think about `.cxtpl` as a lambda-function returning std::string. It is preferable not to use `#include` from `.cxtpl`, just create `.cxtpl.h` file. Then `#include` both generated `.cxtpl.cpp` and the created `.cxtpl.h` in your app code.

Code generated from `.cxtpl` must create a variable with the name `cxtpl_output`, so structure your code as below:
```
/// note: header is NOT generated, it includes stuff for other generated file
...
You need to `#include` all headers used by the template generator in your app code. It is good practice to create a separate `.cxtpl.h` file near to your `.cxtpl`. Separation of the include files allows you to use same includes/logic both in compile-mode (just `#include` your `.cxtpl.h`) and cling-mode (pass contents of your `.cxtpl.h` as function argument). See `enum_gen_hpp.cxtpl.h` and `CXTPL_STD.h` as example.
...
Example of log configuration which writes to both the file and console stream:
```
...
`--single_task_timeout_ms` to limit execution time for a single input file (performs check after task completion to stop other tasks from running).
